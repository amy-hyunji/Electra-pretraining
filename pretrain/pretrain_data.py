...
def get_input_fn(config: configure_pretraining.PretrainingConfig, is_training,
                 num_cpu_threads=4, filename="None"):
  """Creates an `input_fn` closure to be passed to TPUEstimator."""

  input_files = []
  if filename == "random":
     for input_pattern in config.random_tfrecords.split(","):
       input_files.extend(tf.io.gfile.glob(input_pattern))
  elif filename == "wordnet":
     for input_pattern in config.wordnet_tfrecords.split(","):
        input_files.extend(tf.io.gfile.glob(input_pattern)) 
#  print(input_files[0]) - ./dataset/wordnet_tfrecords/pretrain_data.tfrecord-1-of-1000
  ...
  def input_fn(params):
    ...
    name_to_features = {
        ...
        "label": tf.io.FixedLenFeature([config.max_seq_length], tf.int64)
	 }
    
    b = tf.data.Dataset.from_tensor_slices(tf.constant(input_files))
    b = b.repeat()
    b = b.shuffle(buffer_size=len(input_files))
